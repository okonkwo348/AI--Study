1. Write short, honest answers to these questions:
    a. How have you used AI for coding so far?

    - I have used AI for debugging an error when building 
    a school management system at the model.py file. 

    - Do you usually try problems yourself first?
    yes, but not always. Sometimes I just use AI to save me stress and time.


     Can you explain code you've submitted without AI's help?

    - Yes, I have submitted a code that print 'Z'.

     What would happen if AI was unavailable during an exam or job test?

    - I would try using my head and writing all I know.

2. Identify your current pattern: 

   - Learner A: uses AI as an answer generator.


3. Write a short paragraph describing where you are today and what type of learner you want to become.

    At the moment I am using AI as an answer generator more than a learning amplifier. Although it makes me solve problems faster but it makes my confidence level on that body of knowledge low. 
    I want to be a learner who uses AI for amplifying my learning by using AI ethically not to replace my thinking and creativity.